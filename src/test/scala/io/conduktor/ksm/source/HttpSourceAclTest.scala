...
class HttpSourceAclTest extends FlatSpec with Matchers with MockFactory with BeforeAndAfterEach {
  ...
  it should "be able to read contents of a HTTP Endpoint" in {
    ...
    httpSourceAcl.configure(wireMockServer.baseUrl() + path, csvAclParser.name, "GET", "NONE", null, null, null)
  ...
  it should "validate GCP authentication token when security is enabled" in {
